...
from datasets.covid.cxrdataset import CXRDataset
...
def grouped_split(dataframe, metadataframe, random_state=None, test_size=0.05):
    ...
    traingroups, testgroups = sklearn.model_selection.train_test_split(
        groups, random_state=random_state, test_size=test_size
    )
    ...
    traindf = pandas.DataFrame.from_dict(
        traindict, orient="index", columns=dataframe.columns
    )
    testdf = pandas.DataFrame.from_dict(
        testdict, orient="index", columns=dataframe.columns
    )
...
def _convert_dataframe(df):
    ...
    new_df = pandas.DataFrame.from_dict(
        new_df_dict, orient="index", columns=columns[1:]
    )
...
class ChestXray14Dataset(CXRDataset):
    def __init__(
        ...
        self.transform = transform  # self._transforms[fold]
        self.path_to_images = os.path.join(root, "ChestX-ray14")
        self.has_appa = False
        self.pneumo = pneumo

        # Load files containing labels, and perform train/valid split if necessary
        labelpath = os.path.join(self.path_to_images, "labels/Data_Entry_2017.csv")
        ...
        if self.fold == "train" or self.fold == "val":
            ...
            # test folds
            foldpath = os.path.join(self.path_to_images, "labels/train_val_list.txt")
            ...
            traindf, valdf, trainmetadf, valmetadf = grouped_split(
                self.df, self.meta_df, random_state=random_state, test_size=0.05
            )
            if self.fold == "train":
                self.df = traindf
            ...
            foldpath = os.path.join(self.path_to_images, "labels/test_list.txt")
            folddf = pandas.read_csv(foldpath, names=["Image Index"])
            foldlist = folddf["Image Index"].values.tolist()
            self.df = self.df.loc[foldlist, :]
            self.meta_df = self.meta_df.loc[foldlist, :]
        ...
        if labels.lower() == "chestx-ray14":
            ...
            raise ValueError(
                'Invalid value of keyword argument "labels": {:s}.'.format(labels)
