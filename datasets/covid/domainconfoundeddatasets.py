...
class DomainConfoundedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # entry 15 -> covid
        label = torch.tensor(item[1][-1])
        return item[0], label, item[2]  # , item[3]
    ...
    def get_all_labels(self):
        ...
        if labels1.shape[0] == 1:
            return numpy.concatenate(
                (self.ds1.get_all_labels(), self.ds2.get_all_labels()), axis=1
            )
        else:
            return numpy.concatenate(
                (self.ds1.get_all_labels(), self.ds2.get_all_labels()), axis=0
            )
