...
class GitHubCOVIDDataset(CXRDataset):
    def __init__(
        self, root, fold, random_state=30493, labels="CheXpert", transform=None
    ):
        """
        ...
        self.path_to_images = os.path.join(root, "GitHub-COVID", "images")
        self.has_appa = False

        # Load files containing labels, and perform train/valid split if necessary
        metadatapath = os.path.join(root, "GitHub-COVID", "metadata.csv")
        self.df = pandas.read_csv(metadatapath)
        self.df = _convert_dataframe(self.df)

        train, test = grouped_split(self.df, random_state=random_state, test_size=0.10)

        train, val = grouped_split(train, random_state=random_state, test_size=0.10)
